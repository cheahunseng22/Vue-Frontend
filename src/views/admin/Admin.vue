...
const email = ref('')
const password = ref('')
const e = ref('') // error message holder
...
const handleAdmin = async () => {
  try {
    ...
    // ✅ Save login token from access_token
    localStorage.setItem('token', response.data.access_token)
    axios.defaults.headers.common['Authorization'] = `Bearer ${response.data.access_token}`

    // ✅ Save flag so dashboard allows access
    ...
    e.value = errorMessage // show on UI
    alert(errorMessage) // optional alert
  }
}
</script>
  ...
  <div class="flex flex-col md:flex-row bg-white rounded-xl shadow-lg overflow-hidden max-w-4xl w-full">
    ...
    <div class="md:w-1/2 p-8 flex flex-col justify-center">
      ...
      <form @submit.prevent="handleAdmin" class="space-y-5 text-gray-800">
        ...
        <p v-if="e" class="text-red-500 text-sm text-center">{{ e }}</p>
      </form>
    </div>
  </div>
</div>
